...
This project automates the creation of cloud images (AWS AMIs, vSphere OVAs, GCP Compute Images, and Azure Shared Image Gallery images) for Replicated Embedded Cluster applications. It simplifies the process of packaging and distributing Replicated applications across multiple cloud platforms, enabling seamless deployment of air-gapped installations.
...
1. Infrastructure account with appropriate permissions (currently supports AWS,
   vSphere, Google Cloud Platform, and Azure)
...
4. `make`, `packer`, `jq`, and `yq`. For AWS, you will need the AWS CLI
   ...
   you will need the `gcloud` CLI tools. For Azure you will need the Azure
   CLI tools. Note: there is a `Brewfile` and a `shell.nix` to help you with
   these dependencies.
...
2. Update `params.yaml` with your AWS/vSphere/GCP/Azure and Replicated credentials:
...
azure:
  ...
  client_id: <YOUR AZURE_CLIENT ID>
  client_secret: <YOUR AZURE CLIENT SECRET>
  sig_resource_group: <RESOURCE GROUP WHERE YOUR SHARED IMAGE GALLERY IS PROVISIONED>
  shared_image_gallery: <SHARED IMAGE GALLERY WHERE IMAGE WILL BE PUBLISHED>
  replication_regions:
    - eastus
    - centralus
```
...
to build a GCP image

```
make gcp:APP_SLUG/CHANNEL_SLUG
```

and to build an Azure image

```
make azure:APP_SLUG/CHANNEL_SLUG
```
...
make -qp | grep -E '^ova:'
make -qp | grep -E '^gcp:'
make -qp | grep -E '^azure:'
```

This will list all the dynamically generated `ami:APP_SLUG/CHANNEL_SLUG`,
`ova:APP_SLUG/CHANNEL_SLUG`, `gcp:APP_SLUG/CHANNEL_SLUG`, and
`azure:APP_SLUG/CHANNEL_SLUG` targets based on your current Replicated
...
- **Azure**: Hosts the resulting Compute Gallery image in a specified Shared Image Gallery and allows for multi-region distribution
...
7. For AMIs, they are shared to specified AWS regions and accounts. For OVAs,
   ...
   in your GCP project. For Azure images they are shared to the specified regions.
...
- **Distribution**:
  ...
  - Azure images are shared in the specified shared image gallery and replicated across the specified regions. Sharing to other accounts or on the marketplace is controlled by RBAC settings on your shared image gallery.
...
vSphere, GCP, and Azure platforms.
